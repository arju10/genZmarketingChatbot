...
# Import the utility functions for prompt sanitization and validation
from prompt_utils import sanitize_input, validate_input
...
if user_prompt:
    ...
    if not validate_input(sanitized_prompt):
        st.error("Your query contains forbidden or suspicious content. Please rephrase and try again.")
    elif "vectors" in st.session_state:
        document_chain = create_stuff_documents_chain(llm, prompt)
        retriever = st.session_state.vectors.as_retriever(search_kwargs={"k": 5})
        retrieval_chain = create_retrieval_chain(retriever, document_chain)
        response = retrieval_chain.invoke({'input': sanitized_prompt})
        st.write(response['answer'])
        with st.expander("Document Similarity Search"):
            for doc in response['context']:
                st.write(doc.page_content)
